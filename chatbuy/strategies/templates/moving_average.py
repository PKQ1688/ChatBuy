...
class MovingAverageCrossStrategy(BaseStrategy):
    ...
    def validate_parameters(self) -> bool:
        ...
        return (
            isinstance(self.fast_period, int) and 
            isinstance(self.slow_period, int) and
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.Series:
        ...
        buy_condition = (df["fast_ma"] > df["slow_ma"]) & (df["fast_ma"].shift(1) <= df["slow_ma"].shift(1))
        signals[buy_condition] = 1
        
        # Sell signal: fast MA crosses below slow MA
        sell_condition = (df["fast_ma"] < df["slow_ma"]) & (df["fast_ma"].shift(1) >= df["slow_ma"].shift(1))
